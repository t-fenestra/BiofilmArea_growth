LinearRegresionFit_perLayer<-function(df,file_prefix){
  fit_lm_layer<-function(df) lm(log(SumSliceY)~frame,data=df)
  ...
  by_experiment <- by_experiment %>%
    mutate(model = map(data, fit_lm_layer)) %>%
  ...
  DataPredict<-by_experiment %>%
    ...
    mutate(res=pred-SumSliceY) %>%
    gather(key=Area_type,value=Area,SumSliceY,pred)
  ...
  A<-left_join(FitCoeff,p,by="Experiment")
